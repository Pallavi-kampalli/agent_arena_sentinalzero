...
5. **`policies.csv`**
   ...
   - Columns: `id`, `title`, `category`, `version`, `updated_at`, `is_authoritative`, `content`.
   - Access at runtime via: `tools.get_document(document_id)` or `tools.search_knowledge(query)`.

6. **`previous_cases.csv`**
   - Historical support tickets and resolutions (crucial for `previous_agent_was_wrong` investigations).
   - Columns: `case_id`, `customer_id`, `date`, `category`, `resolution`, `agent_id`, `notes`, `was_correct`, `evidence_used`.
